$(document).ready(function(e){
    ...
    $('body').on('click','.tr-solicitud-paciente',function (e){
        window.open(base_url+'triage/generar_documento?t='+$(this).data('id'), '_blank');
    })
...
//                    });
                        ...
                        $.ajax({
                            ...
                            beforeSend: function (data, textStatus, jqXHR) {
                                ...
                                if($('input[name=triage_solicitud_rx]').val()=='Si'){
                                    window.open(base_url+'triage/solicitar_rx?t='+data.triage_id, '_blank');
                                }else{
                                    window.open(base_url+'triage/generar_documento?t='+data.triage_id, '_blank');
                                }
                                if(data.accion=='1'){
        ...
        $.ajax({
            ...
            beforeSend: function (xhr) {
                ...
                if(data.accion=='1'){
                    ...
                    window.open(base_url+'triage/generar_documento?t='+$('body input[name=triage_id]').val(), '_blank');
                    window.top.close();
                }
            },error: function (jqXHR, textStatus, errorThrown) {
                msj_error_serve()
    ...
    $('body').on('click','.solicitar-estidios-rx',function (e){
//        e.preventDefault();
//        var el=$(this);
//        if(confirm('¿SOLICITAR ESTUDIOS CLINICOS?')){
//            location.href=el.attr('href');
//        }
//      else{
//            $.ajax({
//                url: base_url+"triage/actualizar_solicitud",
//                type: 'POST',
//                dataType: 'json',
//                data:{
//                    'csrf_token':csrf_token,
