   ...
   var infoCirugia = '';
   var fechaTratamiento = '';
   var dateToday = new Date();
   dateToday.setDate(dateToday.getDate());
   var redrawDerecho = false;
   var redrawMedico = false;
   ...
   function ver_cirugias(arguments) {
      ...
      var resultado = ajax('post',base_url+'materiales_consumo/hospitalizacion/ver_cirugia',true,obj,'json');
      resultado.success(function(data){
         switch(data.action) {
            case '1':
               updateFootable(data.cirugias,'#ver-tabla-cirugias',10);
               break;
            case '2':
   ...
   //    var obj = {'csrf_token' : $.cookie('csrf_cookie')};
   //    var resultado = ajax('post',base_url+'materiales_consumo/hospitalizacion/elementos_cirugia',true,obj,'json');
   //    resultado.success(function(data){
   //       switch(data.action) {
   //          case '1':
   ...
   function medico_tratante(arguments) {
      ...
      var resultado = ajax('post',base_url+'materiales_consumo/hospitalizacion/medico_tratante',true,obj,'json');
      resultado.success(function(data){
   ...
   function derechohabiente() {
      ...
      var resultado = ajax('post',base_url+'materiales_consumo/hospitalizacion/derechohabiente',true,obj,'json');
      resultado.success(function(data){
         $('.loading').addClass('fa-search').removeClass('fa-spinner fa-spin');
         switch(data.action) {
            case '1':
   ...
   function cambiar_tab(data) {
      ...
      if (data.accion == 'estado') {
         $.ajax({
            url: base_url+'materiales_consumo/hospitalizacion/estados_cirugia',
            type: 'post',
            dataType: 'json',
            data: {
               'csrf_token' : $.cookie('csrf_cookie'),
   ...
   function getInfoCirugia (data) {
      infoCirugia = data;
      editarCirugia = true;
      $.ajax({
         url: base_url+'materiales_consumo/hospitalizacion/ver_cirugia_editar',
         type: 'post',
         dataType: 'json',
         data: {
            'csrf_token' : $.cookie('csrf_cookie'),
      ...
      .done(function(data) {
         switch(data.action){
            case '1': 
               ...
               $('#medico-tratante').data('matricula',data['cirugia']['matricula']);
               // $('#fecha-tratamiento').datepicker('setDate',new Date(data['cirugia']['fecha']));
               $('#fecha-tratamiento').val(data['cirugia']['fecha']);
               $('#diagnostico').val(data['cirugia']['diagnostico']);
               $('#destino').html(data['cirugia']['pisosOptions']).select2();
               $('#destino').select2().select2('val',data['cirugia']['idquirofano']);
               var materiales = [];
               elementos_mat = [];
               sis_materiales = [];
   ...
   function insert_cirugia(arguments) {
      $.ajax({
         url: base_url+'materiales_consumo/hospitalizacion/insert_tratamiento',
         type: 'post',
         dataType: 'json',
         data: {
            'csrf_token'         : $.cookie('csrf_cookie'),
            ...
            'fecha' : getFormattedDate(new Date($('#fecha-tratamiento').datepicker('getDate'))),
            'diagnostico' : $('#diagnostico').val(),
            'idquirofano' : $('#destino').val(),
            'matricula' : $('#medico-tratante').data('matricula')
         }
      })
      .done(function(data) {
         switch(data.action) {
   ...
   function editar_cirugia() {
      $.ajax({
         url: base_url+'materiales_consumo/hospitalizacion/editar_cirugia',
         type: 'post',
         dataType: 'json',
         data: {
            'idCirugia'         : idCirugia,
   ...
   function sistemas_mq (id,element_id) {
      $.ajax({
         url: base_url+'materiales_consumo/hospitalizacion/get_materiales_sis',
         type: 'post',
         dataType: 'json',
         data: {
            'id_sis'     : id,
                        ...
                        callback  : function(result) {
                           ...
                           $('#mat_sistema tbody tr').each(function(index, el) {
                              ...
                              if (checked == 1) {
                                 ...
                                 var disponibles = $(this).data('disponibles');
                                 cantidad = parseInt(cantidad);
                                 if ((cantidad < 1 || cantidad > disponibles) && element_id == 'material') {
                                    valid = false;
                                 }
                              }
                           });
                           if (!valid) {
                              elements_m.pop();
                              $('#material').select2().select2('val',elements_m);
                              $('#p-cirugia-form').validate().element($('#material'));
                              alert_error({'msj':'La cantidad a seleccionar debe ser mayor que 0 ó menor/igual que disponibles'});
                           }
                           else {
                              $('#mat_sistema tbody tr').each(function(index, el) {
                                 var checked = $(this).find('input:checked').length;
   ...
   function get_sistemas_m (id_cirugia,selectionArray,select) {
      $.ajax({
         url: base_url+'materiales_consumo/hospitalizacion/sistemas',
         dataType: 'json'
      })
      .done(function(data) {
         switch(data.action) {
            case '1':
               if (selectionArray != undefined && select == 'materiales') {
                  ...
                  $('#material').html(data['html_material']).select2(); 
                  $('#destino').html(data['pisos']).select2(); 
   ...
   function eliminarCirugia (cirugia) {
      $.ajax({
         url: base_url + 'materiales_consumo/hospitalizacion/eliminarCirugia',
         type: 'post',
         dataType: 'json',
         data: {
            'csrf_token' : $.cookie('csrf_cookie'),
   ...
   function doAccion (accion) {
      $.ajax({
         url: base_url + 'materiales_consumo/hospitalizacion/doAccion',
         type: 'POST',
         dataType: 'json',
         data: {
            csrf_token : $.cookie('csrf_cookie'),
   ...
   $('.date .input-group-addon').on('click', function(event) {
      event.preventDefault();
      $('#fecha-tratamiento').focus();
   });

   $('#fecha-tratamiento').on('change', function(event) {
      event.preventDefault();
      $('#p-cirugia-form').validate().element($('#fecha-tratamiento'));
   });
   ...
   $('#ver-a').on('click',function(){
      ...
      $('#estado-a').addClass('no-display');
      ver_cirugias();
      // window.location.replace(base_url + 'materiales_consumo/hospitalizacion/gestionar_cirugia');
   });
   ...
   $('#ver-tabla-cirugias').on('click','.acciones',function(){
      ...
      if (data.accion == 'modificar' || data.accion == 'estado') {
         ...
         if (!$(this).hasClass('icono-disable')) {
            modalAccion(data);
         }
      }
      else if (data.accion == 'ver-documento') {
         var tipoDoc = $(this).parents('tr').data('tipo-documento');
         switch(tipoDoc) {
            case 1: window.open(base_url+redirect_solicitud_consumo+'/'+$(this).parents('tr').data('solicitud'),'_blank' ); break;
         }
      }
   });
      ...
      dialog({
         ...
         'pre_tabla'  : 
            '<table class="footable table-sig table-hover table-condensed" data-page-size="5">'+
               '<thead>'+
                  '<tr>'+
                     '<th data-sort-ignore="false" data-toggle="true" >NSS</th>'+
                     '<th>Nombre</th>'+
                     '<th>Paterno</th>'+
                     '<th>Materno</th>'+
                  '</tr>'+
   ...
   $('#cancelar').on('click',function(){
      ...
      if (!$(this).parents('form').hasClass('alta-cirugia')) {
         getInfoCirugia(infoCirugia);
      }
   });
   ...
   .on('changeDate', function(event) {
      // fechaTratamiento = getFormattedDate(new Date($('#fecha-tratamiento').datepicker('getDate')));
   });
