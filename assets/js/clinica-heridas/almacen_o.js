  $('document').ready(function() {
   ...
   function ver_a_revision (mensaje) {
      $.ajax({
         url: base_url+'materiales_consumo/almacen_osteo/ver_a_revision',
         dataType: 'json'
      })
      .done(function(data) {
         switch(data.action) {
            case '1': 
               updateFootable(data.tr,'#ver-tabla-a-revision',10);
               break;
            case '2': 
               if (mensaje) {
                  alert_error(data.msj);
               }
               updateFootable('','#ver-tabla-a-revision',10);
               break;
    ...
    function detalles_material (data) {
      bootbox.dialog({
         ...
         message: 
         '<table id="'+data['id_tabla']+'" class="footable table-sig table-hover table-condensed" data-page-size="5">'+
               '<thead>'+
                  data['thead']+
               '</thead>'+
               '<tbody>'+
         ...
         buttons: {
            success: {
               ...
               callback  : function(result) {
                  ...
                  var cantidadActual = $('#id-tabla-nuevo').find('tbody tr').data('cantidad');
                  if (cantidad != '') {
                     updateMateriales({
   ...
   function updateMateriales (info) {
      $.ajax({
         url: base_url + 'materiales_consumo/almacen_osteo/updateMateriales',
         type: 'post',
         dataType: 'json',
         data: {
            ...
            cantidadActual : info.cantidadActual,
            accion : accion
         }
      })
      .done(function(data) {
   ...
   function get_sis_archivo (id_cirugia) {
      $.ajax({
         url: base_url+'materiales_consumo/almacen_osteo/detalles_archivo',
         type: 'post',
         dataType: 'json',
         data: {
            'csrf_token' : $.cookie('csrf_cookie'),
   ...
   function agregar_entrega (id_cirugia) {
      $.ajax({
         url: base_url+'materiales_consumo/almacen_osteo/get_id_usuario',
         dataType: 'json'
      })
      .done(function(data) {
         switch(data.action) {
               ...
               bootbox.dialog({
                  ...
                  buttons: {
                     success: {
                        ...
                        callback: function () {
                           $.ajax({
                              url: base_url+'materiales_consumo/almacen_osteo/por_entregar_ci',
                              type: 'post',
                              dataType: 'json',
                              data: {
                                 'csrf_token' : $.cookie('csrf_cookie'),
   ...
   function continuar (id_cirugia,funcion_ci) {
      $.ajax({
         url: base_url+'materiales_consumo/almacen_osteo/'+funcion_ci,
         type: 'post',
         dataType: 'json',
         data: {
            'csrf_token' : $.cookie('csrf_cookie'),
   ...
   function ver_por_entregar (mensaje) {
      $.ajax({
         url: base_url+'materiales_consumo/almacen_osteo/ver_por_entregar',
         dataType: 'json'
      })
      .done(function(data) {
         switch(data.action) {
   ...
   function ver_sin_existencia (mensaje) {
      $.ajax({
         url: base_url+'materiales_consumo/almacen_osteo/ver_sin_existencia',
         dataType: 'json'
      })
      .done(function(data) {
         switch(data.action) {
            case '1': 
               updateFootable(data.tr,'#ver-tabla-sin-existencia',pagination);
               break;
            case '2': 
               if (mensaje) {
                  alert_error(data.msj);
               }
               updateFootable('','#ver-tabla-sin-existencia',pagination);
               break;
            case '3': expired_crf(); break;
         }
      });
   ...
   function ver_archivo (mensaje) {
      $.ajax({
         url: base_url+'materiales_consumo/almacen_osteo/ver_archivo',
         dataType: 'json'
      })
      .done(function(data) {
         switch(data.action) {
            case '1':
               updateFootable(data.tr,'#ver-tabla-archivo',pagination); 
               break;
            case '2': 
               if (mensaje) {
                  alert_error(data.msj);
   ...
   function gestion_procesos (mensaje) {
      $.ajax({
         url: base_url+'materiales_consumo/almacen_osteo/ver_gestion_procesos',
         dataType: 'json'
      })
      .done(function(data) {
         switch(data.action) {
            case '1': 
               updateFootable(data.tr,'#ver-tabla-gestion-i',pagination);
               break;
            case '2': 
               if (mensaje) {
                  alert_error(data.msj);
               }
               updateFootable('','#ver-tabla-gestion-i',pagination);
               break;
            case '3': expired_crf(); break;
         }
      });
   ...
   function ver_detalles_material (id_sistema) {
      ...
      $.ajax({
         url: base_url+'materiales_consumo/almacen_osteo/detalles_material_g',
         type: 'post',
         dataType: 'json',
         data: {
            'csrf_token'      : $.cookie('csrf_cookie'),
               ...
               detalles_material({
                  thead : 
                     '</tr>'+
                        ...
                        '<th>Máxima</th>'+
                        '<th>Mínima</th>'+
                        '<th data-sort-ignore="true" class="text-center">Acción</th>'+
                     '</tr>',
                  tbody : data['tr'],
   ...
   function material_nuevo (datos) {
      ...
      accion = datos.accion;
      $.ajax({
         url: base_url+'materiales_consumo/almacen_osteo/consultar_materiales',
         type: 'post',
         dataType: 'json',
         data: {
            'csrf_token'   : $.cookie('csrf_cookie'),
               ...
               detalles_material_nuevo({
                  thead : 
                     '<tr>'+
                        ...
                        '<th>Contrato</th>'+
                        '<th>Cantidad</th>'+
                        '<th data-sort-ignore="true" class="text-center">Cantidad</th>'+
                     '</tr>',
                  tbody : data['tr'],
   ...
   $('body').on('click', '#id-tabla-detalles .acciones', function(event) {
      ...
      material_nuevo({
         ...
         idSistema : $(this).parents('tr').data('id-sistema'),
   ...
   $('body').on('click', '.buscar-empleado', function(event) {
      event.preventDefault();
      $.ajax({
         url: base_url+'materiales_consumo/almacen_osteo/empleado_ceye',
         dataType: 'json'
      })
      .done(function(data) {
         switch(data.action) {
            case '1': 
               bootbox.dialog({
                  ...
                  message: 
                     '<div>'+
                        '<table id="empleados-ceye" class="footable table-sig table-hover table-condensed" data-page-size="10">'+
                           '<thead>'+
                              '<tr>'+
   ...
   $('#ver-tabla-a-revision').on('click', '.acciones', function(event) {
      ...
      switch(accion) {
         ...
         case 'codigo_barra': window.open($(this).parent('a').attr('href'));
      }
   });
